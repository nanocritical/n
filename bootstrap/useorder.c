...
static error fwd_declare_types_each(struct module *mod, struct node *node,
  ...
  case DEFINTF:
    if (td & TD_DYN_NEEDS_TYPE) {
      ...
      st->mask_state->inv_mask = ~(TD_FUN_NEEDS_TYPE | TD_DYN_NEEDS_TYPE | TD_FUNBODY_NEEDS_DYNBODY);
...
static error fwd_define_dyns_each(struct module *mod, struct node *node,
  ...
  if ((td & (TD_TYPEBODY_NEEDS_DYN | TD_FUNBODY_NEEDS_DYN | TD_FUNBODY_NEEDS_DYNBODY))
...
//  debug = strcmp(mod->filename, "lib/n/builtins/builtins.n")==0;
